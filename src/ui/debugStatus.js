...
function getStatusLines(assets, worldId, stage, stageRoot, inspectedCell) {
  ...
  const lines = [
    ...
    `player: ${levelState ? `${levelState.player.x}/${levelState.player.y} ${levelState.player.direction}` : "-"}`,
    `player health: ${levelState ? `${levelState.player.health}/${levelState.player.maxHealth}` : "-"}`,
    `player lives: ${levelState?.player.lives ?? "-"}`,
    `game over: ${levelState?.player.gameOver ? "yes" : "no"}`,
    `player frame: ${levelState?.player.spriteFrameId || "-"}`,
